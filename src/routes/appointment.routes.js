  ...
  for (const existing of existingList) {
    if (
      overlaps(
        requestedStart,
        durationMinutes,
        parseTimeString(existing.time),
        Number(existing.durationMinutes || 30)
      )
    ) {
...
async function ensurePatientNotDoubleBooked({
  date,
  time,
  durationMinutes,
  patientId,
...
async function createAppointment(payload) {
  if (
    ...
    throw new Error(
      'patientName, email, phone, idNumber, date, time, serviceId, serviceName, slotId, dentistId, dentistName, branchId and branchName are required'
    );
  }
  ...
  await ensurePatientNotDoubleBooked({
    date: payload.date,
    time: payload.time,
    durationMinutes: Number(payload.durationMinutes || 30),
  ...
  const appointment = await Appointment.create({
    ...
    auditTrail: [
      {
        action: 'CREATED',
        byUserId: payload.bookedByUserId || null,
        byRole: payload.bookedByRole || 'SYSTEM',
...
router.post('/', optionalAuthenticate, async (req, res) => {
  try {
    ...
    return res
      .status(
        error.message.match(/required|past|selected|Duration|overlapping|outside/) ? 400 : 500
      )
...
router.patch('/mine/:id', authenticate, authorize('PATIENT'), async (req, res) => {
  try {
    ...
    addAudit(appointment, {
      ...
      byUserId: req.user.id,
      byRole: req.user.role,
      note: req.body.auditNote || 'Appointment updated by patient'
    });
    await appointment.save();
    ...
    return res
      .status(error.message.match(/past|selected|Duration|overlapping|outside|moved/) ? 400 : 500)
      .json({ message: error.message });
  }
});
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (
        !ensureFutureDate(appointment.date) &&
